...
class Net(pl.LightningModule):
    ...
    def __init__(self, trial, input_dim=133, output_dim=3):
        #super().__init__()
        #self.fc1 = nn.Linear(133, 80) #辞書の数→中間層2の数
        ##self.fc2 = nn.Linear(100, 3)    #中間層の数→クラス数
        #self.fc2 = nn.Linear(80, 30)    #中間層2の数→中間層3の数
        #self.fc3 = nn.Linear(30, 3)    #中間層3の数→クラス数

        #(226, 133) (97, 133) BoW

        super().__init__()
        self.hidden_dim_1 = trial.suggest_int("hidden_dim_1", 16, 256, log=True)
        self.hidden_dim_2 = trial.suggest_int("hidden_dim_2", 16, 256, log=True)
        self.layers = nn.Sequential(
            nn.Linear(input_dim, self.hidden_dim_1),
            nn.ReLU(),
            nn.Linear(self.hidden_dim_1, self.hidden_dim_2),
            nn.ReLU(),
            nn.Linear(self.hidden_dim_2, output_dim)
        )
...
def objective(trial):
    ...
    # モデルのインスタンス化
    model = Net(trial, input_dim, output_dim)

    # Trainerの設定
    trainer = pl.Trainer(max_epochs=30)  # 必要に応じてデバイスの指定やトレーニング設定を変更してください
...
# #------------------
# # 学習済みモデルの保存
# torch.save(net.state_dict(), 'NPmodel.pt')
...
# net = Net()
# net.load_state_dict(torch.load('NPmodel.pt'))
